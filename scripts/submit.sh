...
if [ "$(git rev-list --count "$BASE_REF"..HEAD 2>/dev/null || echo 0)" = "0" ]; then
    die "Something to submit" "Your branch has no changes for ARIA to review. Edit workspace/triage.yml and workspace/eradicate.yml and run 'make test' until all phases pass, then run 'make submit' again"
fi
...
if [ -n "${EXISTING:-}" ]; then
    ...
    PR_OUT=$(gh pr create --base main --head "$BRANCH" \
        ...
        --body "Lieutenant submission for ARIA review. Run \`make test\` locally before requesting review." 2>&1)
    if [ $? -ne 0 ]; then
